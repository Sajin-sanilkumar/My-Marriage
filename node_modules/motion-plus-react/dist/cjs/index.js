...
function Ticker({ items, velocity = 50, hoverFactor = 1, gap = 10, axis = "x", align = "center", offset, isStatic = false, ...props }) {
    ...
    const isInViewport = react.useInView(containerRef, { margin: "100px" });
    ...
    for (let i = 0; i < cloneCount; i++) {
        items.forEach((item, itemIndex) => {
            ...
            const cloneBounds = originalBounds
                ...
                : defaultBounds;
            clonedItems.push(jsxRuntime.jsx(TickerItem, { offset: renderedOffset, axis: axis, listSize: totalListSize, cloneIndex: itemIndex, bounds: cloneBounds, containerPaddingStart: state.containerPaddingStart, alignItems: alignItems, children: item }, `clone-${i}-${itemIndex}`));
        });
    }
    useFocusNavigation(containerRef, axis, focusOffset, offset, setHasFocus);
    return (jsxRuntime.jsx(ListView, { containerProps: props, containerRef: containerRef, listRef: listRef, gap: gap, axis: axis, alignItems: alignItems, isMeasured: isMeasured, isInView: isInView, renderedOffset: renderedOffset, items: items, clonedItems: clonedItems, onPointerEnter: () => {
...
function ListView({ containerProps, containerRef, listRef, gap, axis, alignItems, isMeasured, isInView, isStatic, items, clonedItems, renderedOffset, onPointerEnter, onPointerLeave, totalListSize, state, }) {
    return (jsxRuntime.jsx(react.motion.div, { ...containerProps, ref: containerRef, style: { ...containerStyle, ...containerProps.style }, onPointerEnter: onPointerEnter, onPointerLeave: onPointerLeave, children: jsxRuntime.jsxs(react.motion.ul, { ref: listRef, style: {
            ...
            }, children: [items.map((item, index) => (jsxRuntime.jsx(TickerItem, { axis: axis, offset: renderedOffset, listSize: totalListSize, itemIndex: index, bounds: state.itemSizes[index] ?? defaultBounds, containerPaddingStart: state.containerPaddingStart, alignItems: alignItems, children: item }, "original-" + index))), clonedItems || null] }) }));
}
