import { useTransform, motion, isHTMLElement, wrap, frame, useMotionValue, useInView, usePageInView, useReducedMotion, useIsomorphicLayoutEffect, resize, useAnimationFrame, noop, animate } from 'framer-motion';
 ...
 */
function TickerItem({ offset, axis, listSize = 0, itemIndex, cloneIndex, bounds, containerPaddingStart, alignItems, ...props }) {
    ...
    return (<TickerItemContext.Provider value={{ offset: itemOffset }}>
            ...
            flexGrow: 0,
            flexShrink: 0,
            height: axis === "x" ? itemWrapperOffAxisSize : undefined,
            width: axis === "y" ? itemWrapperOffAxisSize : undefined,
    ...
    for (let i = 0; i < cloneCount; i++) {
        items.forEach((item, itemIndex) => {
            ...
            const cloneBounds = originalBounds
                ...
                : defaultBounds;
            clonedItems.push(<TickerItem key={`clone-${i}-${itemIndex}`} offset={renderedOffset} axis={axis} listSize={totalListSize} cloneIndex={itemIndex} bounds={cloneBounds} containerPaddingStart={state.containerPaddingStart} alignItems={alignItems}>
                    {item}
                </TickerItem>);
        });
    }
    useFocusNavigation(containerRef, axis, focusOffset, offset, setHasFocus);
    return (<ListView containerProps={props} containerRef={containerRef} listRef={listRef} gap={gap} axis={axis} alignItems={alignItems} isMeasured={isMeasured} isInView={isInView} renderedOffset={renderedOffset} items={items} clonedItems={clonedItems} onPointerEnter={() => {
            animate(velocityFactor, hoverFactor);
        }} onPointerLeave={() => {
            animate(velocityFactor, 1);
        }} totalListSize={totalListSize} state={state}/>);
}
function ListView({ containerProps, containerRef, listRef, gap, axis, alignItems, isMeasured, isInView, isStatic, items, clonedItems, renderedOffset, onPointerEnter, onPointerLeave, totalListSize, state, }) {
    return (<motion.div {...containerProps} ref={containerRef} style={{ ...containerStyle, ...containerProps.style }} onPointerEnter={onPointerEnter} onPointerLeave={onPointerLeave}>
            ...
            willChange: isMeasured && isInView ? "transform" : undefined,
                ...
                {items.map((item, index) => (<TickerItem key={"original-" + index} axis={axis} offset={renderedOffset} listSize={totalListSize} itemIndex={index} bounds={state.itemSizes[index] ?? defaultBounds} containerPaddingStart={state.containerPaddingStart} alignItems={alignItems}>
                        {item}
                    </TickerItem>))}
                {clonedItems || null}
            </motion.ul>
