...
function CategoryTable({ data, origin, loading, weddingName }: {
              ...
              {loading
                ...
                : data.map((row) => {
                    ...
                    const waMsg  = encodeURIComponent(
                      `🎉 You're invited to ${weddingName}'s wedding!\nSee details & RSVP: ${inviteUrl}`
                    );
...
function VipTable({ data, origin, loading, weddingName }: {
  data: Analytics["not_responded_vip"]; origin: string; loading: boolean; weddingName: string;
}) {
  return (
    <section>
                ...
                {loading
                  ...
                  : data.map((guest) => {
                      ...
                      const msg = encodeURIComponent(
                        `Dear ${guest.guest_name}, you are warmly invited to ${weddingName}'s wedding!\n\nDetails & RSVP: ${inviteUrl} 🙏`
                      );
...
export default function OverviewPage() {
  ...
  const [weddingName, setWeddingName] = useState("the wedding");
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    setError(null);
    try {
      const [analyticsRes, weddingRes] = await Promise.all([
        fetch(`/api/weddings/${WEDDING_SLUG}/analytics`),
        fetch(`/api/weddings/${WEDDING_SLUG}`),
      ]);
      if (!analyticsRes.ok) throw new Error(`${analyticsRes.status}: ${analyticsRes.statusText}`);
      setData(await analyticsRes.json());
      if (weddingRes.ok) {
        const w = await weddingRes.json();
        setWeddingName(`${w.bride_name} & ${w.groom_name}`);
      }
    } catch (e) {
    ...
    <div className="p-6 md:p-8 space-y-8 pb-24 md:pb-8">
      ...
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-1">
          Live summary for {weddingName}'s wedding.
        </p>
      </div>
